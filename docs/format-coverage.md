...
| `properties`     | ⚠ `gap` — `SetPropertyHeader`/`AddProperty` opaque  | ⚠ gap          | ✓ user properties + `group` + `perspectives.<name>` |
...
| -------------------------------------- | ------------------------------------------------ | --------------------------------------------------------- |
| Container/Relation `properties` (PUML) | none — drop'аем при load                         | Custom parser pass для `SetPropertyHeader`/`AddProperty`  |
