...
const buildBoundaryLookups = (
    ...
    const nameSets = new Map(
        boundaries.map((b) => [
            b.name,
            new Set(b.containers.map((c) => c.name)),
        ]),
    );
...
const analyzeModel = (
    ...
    const apiTechnologies =
        options?.apiTechnologies ?? DEFAULT_API_TECHNOLOGIES;
    ...
    for (const boundary of model.boundaries) {
        const { nameSet, childNames, parentBoundary } = lookups.get(
            boundary.name,
        )!;
