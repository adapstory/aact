...
| Function-call argument | `MacroName(...)` or `FuncName()` as an argument value | C4-PUML stdlib uses inline function calls as argument values: e.g. `Rel(A, B, "x", $index=Index())` (where `Index()` is `C4.puml:1713`), or `AddElementTag(..., $shape=RoundedBoxShape())` (`C4.puml:980`). The parser MUST accept function-call expressions as argument values, not just literals.                                                                  |
...
| `SHOW_LEGEND($hideStereotype="true", $details="false")`, `SHOW_FLOATING_LEGEND($alias="", ...)`, `SHOW_DYNAMIC_LEGEND($alias="", ...)`, `SHOW_ELEMENT_TYPE`, `SHOW_PERSON_SPRITE`, `SHOW_PERSON_PORTRAIT`, `SHOW_PERSON_OUTLINE`         | Legend / element-type visibility toggles (`C4.puml:1576-1603`, `C4_Context.puml:335-355`).                                                                                               |
...
| `AddProperty($name, $value)`                                                                                                                                                                                                             | Adds a property row to the next Element (`C4.puml`). Eventual destination is `Model.containers[*].properties`; in the new parser we may surface this as load-bearing once the AST is in. |
