...
| **Archetypes (in scope)**  | alias declarations (`<alias> = <baseKeyword> [{ defaults? }]`) and kind-default declarations (`<baseKeyword> { defaults }`) extracted pre-parse. Element decls that match an alias or kind get archetype defaults merged onto them: description / technology fallback (source wins), tags / properties / perspectives additive. Chained aliases resolve recursively. Relationship archetypes (`https = -> { … }`) and selector-body propagation are documented gaps. | `archetypes { application = container { tag "Application" } }` → `api = application "X"` lands as Container with tag "Application"; `archetypes { softwareSystem { description "Default" } }` → `a = softwareSystem "A"` lands with description "Default"                  |
...
- **Archetype alias usage** — pre-parse pass extracts
  ...
  positionals win); tags / properties / perspectives are additive
  (archetype defaults + header defaults). Verified against
  `DslTests.test_archetypes` — Customer API in the reference fixture
  carries tags `["Application", "Spring Boot"]` and technology
  `Spring Boot`, identical to our output.
- **Archetype kind-default form** — `archetypes { softwareSystem {
description "…" } }` (no alias name on LHS) applies the body to
  every declaration of the named base kind. Verified against
  `DslTests.test_archetypesForDefaults` — every `softwareSystem`
  declaration gets Default Description + Default Tag + Default
  Property + Default Perspective. Same extract pipeline as alias
  decls; differentiated by absence of `Equals` after the head token.
- **Archetype property / perspective propagation** — `properties { … }`
  and `perspectives { … }` blocks inside an archetype body are
  reconstructed as synthetic body statements on every element using
  the alias (or kind-default), so toModel's normal aggregation merges
  them into `Element.properties`.
