...
describe("Adapstory incubating architecture rules", () => {
  it("requires Widget Lake contract evidence for plugin UI surfaces", () => {
    ...
    expect(
      checkAdapstoryWidgetLakeContract(model([legacyPluginUi, widgetLakeUi])),
    ).toMatchObject([
      {
  ...
  it("requires Smart Line surfaces to carry tenant-scope evidence", () => {
    ...
    expect(
      checkAdapstorySmartLineTenantScope(model([smartLine, scopedSmartLine])),
    ).toMatchObject([
      {
  ...
  it("requires tenant isolation evidence for tenant-scoped surfaces", () => {
    ...
    expect(
      checkAdapstoryTenantIsolationEvidence(model([contentApi, pluginGateway])),
    ).toMatchObject([
      {
