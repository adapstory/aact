...
): TestElement => testElement(name, tags, relations);
...
describe("checkAdapstoryNoCoreBcCycles", () => {
  ...
  it("ignores cycles inside one bounded context", () => {
    ...
    expect(checkAdapstoryNoCoreBcCycles(model([api, adapter]))).toHaveLength(0);
  });
  ...
  it("does not ignore legacy dependency-direction tags without a decision matrix entry", () => {
    ...
    expect(
    ...
    ).toMatchObject([
      {
        target: "bc-15",
        targetKind: "boundary",
        message: "core bounded context cycle detected: bc-15 -> bc-19 -> bc-15",
      },
    ]);
  });
